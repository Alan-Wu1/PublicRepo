...
namespace SpOnlineWrapper.Utils
{
    public static class Extensions
    {
        ...
        internal static HttpWebRequest AddBody(this HttpWebRequest request, object body, string contentType = "application/json")
        {
            var toBeEncoded = body is string ? body.ToString() : new JavaScriptSerializer().Serialize(body);
            var bytes = Encoding.ASCII.GetBytes(toBeEncoded);
